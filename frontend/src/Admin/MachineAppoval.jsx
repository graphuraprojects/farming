  ...
  const handleAction = async (actionType) => {
    ...
    if (actionType === 'approve') {
      ...
      if (unchecked.length > 0) {
        if (!confirm(`${unchecked.length} checklist items are not checked. Continue anyway?`)) {
          return;
        }
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fafaf7]">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading machine details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#fafaf7] text-[#2b2b2b] font-['Inter',sans-serif] pb-12 pt-8">
      ...
      {isLightboxOpen && (
        ...
        >
            ...
            >
                <button className="absolute top-4 right-4 z-50 p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600" onClick={closeLightbox}>
                ...
                <div className="relative w-full h-[60vh] md:h-[70vh] bg-gray-50 rounded-xl flex items-center justify-center overflow-hidden">
                    <button className="absolute left-4 z-10 p-3 rounded-full bg-white shadow-lg text-gray-700 hover:text-[#03a74f]" onClick={prevImage}>
                        <span className="material-symbols-outlined text-2xl">chevron_left</span>
                    </button>
                    <img src={machineData.images[currentImageIndex].url} alt="Preview" className="max-w-full max-h-full object-contain"/>
                    <button className="absolute right-4 z-10 p-3 rounded-full bg-white shadow-lg text-gray-700 hover:text-[#03a74f]" onClick={nextImage}>
        ...
        >
            ...
            >
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
                    <h3 className="font-bold text-[#1f3d2b]">Ownership Proof Document</h3>
                    <button className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600" onClick={closeDocViewer}>
                        <span className="material-symbols-outlined text-xl font-bold">close</span>
                    </button>
                </div>
                ...
                <div className="flex-1 bg-gray-50 p-6 flex items-center justify-center overflow-auto">
                    <div className="bg-white p-2 shadow-lg border border-gray-200 rounded-lg">
                        <img src={selectedDoc} className="max-w-full max-h-[600px] object-contain" alt="Ownership Proof" />
                    </div>
                </div>
            </div>
        </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2 pb-4 border-b border-gray-200">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full border ${statusInfo.color}`}>
                  {statusInfo.label}
                </span>
              </div>
              <p className="text-[#5b6e58] text-base">
                {machineData.model} • {machineData.model_year} • Reg: {machineData.registration_no}
              </p>
            </div>
            ...
            {machineData.images && machineData.images.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-[#1f3d2b]/10 p-1 overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-1 md:gap-2">
                  ...
                  >
                    <img src={machineData.images[0].url} className="w-full h-full object-cover" alt="Main" />
                  ...
                  {machineData.images.slice(1, 5).map((img, idx) => (
                    <div key={idx} className="relative group cursor-pointer overflow-hidden rounded-lg h-32 md:h-auto bg-gray-100" onClick={() => openLightbox(idx + 1)}>
                        <img src={img.url} className="w-full h-full object-cover" alt={`Image ${idx + 2}`} />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors"></div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[#1f3d2b]/10 overflow-hidden">
              ...
              <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-6">
                <div>
                  <p className="text-[#5b6e58] text-xs uppercase font-semibold mb-1">Category</p>
                  <p className="text-[#2b2b2b] font-medium">{machineData.category}</p>
                </div>
                <div>
                  <p className="text-[#5b6e58] text-xs uppercase font-semibold mb-1">Fuel Type</p>
                  <p className="text-[#2b2b2b] font-medium">{machineData.fuel_type}</p>
                </div>
                <div>
                  <p className="text-[#5b6e58] text-xs uppercase font-semibold mb-1">Price Per Hour</p>
                  <p className="text-[#2b2b2b] font-medium">₹{machineData.price_per_hour}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-[#1f3d2b]/10 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-[#fafaf7]">
                  <h3 className="font-bold text-lg text-[#1f3d2b]">Ownership Document</h3>
                </div>
                <div className="p-4">
                  {machineData.ownership_proof ? (
                    <div className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors rounded-lg border border-gray-200">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-[#2b2b2b]">Ownership Proof</span>
                      </div>
                      <button 
                        onClick={() => openDocViewer(machineData.ownership_proof.url)}
                    ...
                    <p className="text-gray-500 text-sm p-4">No document uploaded</p>
                  )}
                </div>
              </div>
                ...
                <div className="p-6">
                  {machineData.address ? (
                    <div className="space-y-2 text-sm">
                      <p className="text-gray-700">{machineData.address.street}</p>
                      <p className="text-gray-700">{machineData.address.city}, {machineData.address.state} {machineData.address.zip}</p>
                      <p className="text-gray-700">{machineData.address.country}</p>
                    </div>
                  ) : (
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{machineData.owner_id?.email}</p>
                  </div>
                </div>
            ...
            {machineData.rejection_reason && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-6">
                ...
                <p className="text-sm text-red-700">{machineData.rejection_reason}</p>
              </div>
            )}
            ...
            {!machineData.isApproved && !machineData.rejection_reason && (
              <div className="bg-white rounded-xl shadow-sm border border-[#1f3d2b]/10 p-6 sticky top-24">
                <div className="flex items-center gap-2 mb-4">
                  <span className="material-symbols-outlined text-[#03a74f]">checklist</span>
                ...
                <div className="space-y-3 mb-6">
                  {checklist.map((item) => (
                    <label key={item.id} className="flex items-start gap-3 cursor-pointer group">
                      ...
                      />
                      <span className={`text-sm ${item.checked ? 'text-[#111813]' : 'text-gray-500'}`}>
                        {item.label}
                      </span>
                    </label>
                  ))}
                ...
                <div className="mb-6">
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">
                    Admin Notes {!machineData.isApproved && !machineData.rejection_reason && <span className="text-red-500">*</span>}
                  </label>
                  <textarea 
